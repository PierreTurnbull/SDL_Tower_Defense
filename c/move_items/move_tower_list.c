void  move_tower_list(void)
{
  ...
  while (ptr != NULL) {
    if (ptr->state[TOWER_STATE_GRAB] == TOWER_STATE_GRABBED) {
      ...
      ptr->range_pos.x  = ptr->pos.x - game.play.tower_list.tower_range[ptr->type] + ptr->pos.w / 2;
      ptr->range_pos.y  = ptr->pos.y - game.play.tower_list.tower_range[ptr->type] + ptr->pos.h / 2;
    }
    ptr = ptr->next;
  }
}
